...
DROP POLICY IF EXISTS "Users can view own vendor data" ON vendors;
DROP POLICY IF EXISTS "Cars belong to vendor" ON cars;
DROP POLICY IF EXISTS "Car images belong to vendor" ON car_images;
DROP POLICY IF EXISTS "Car documents belong to vendor" ON car_documents;
DROP POLICY IF EXISTS "Sales belong to vendor" ON sales;
...
DROP POLICY IF EXISTS "Users can upload their own car documents" ON storage.objects;
DROP POLICY IF EXISTS "Users can view their own car documents" ON storage.objects;
...
CREATE POLICY "Users can view their own car images" ON storage.objects 
FOR SELECT USING (
  bucket_id = 'car-images' AND auth.uid()::text = (storage.foldername(name))[1]
);
...
CREATE POLICY "Users can upload their own car documents" ON storage.objects 
FOR INSERT WITH CHECK (
  bucket_id = 'car-documents' AND auth.uid()::text = (storage.foldername(name))[1]
);

CREATE POLICY "Users can view their own car documents" ON storage.objects 
FOR SELECT USING (
  bucket_id = 'car-documents' AND auth.uid()::text = (storage.foldername(name))[1]
);
